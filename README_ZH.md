...
从 [Code Go 下载页](https://shu26.cfd/download) 下载最新版本的 `CodeGo_{版本号}_x64_en-US.msi` 安装包或 `CodeGo_{版本号}_x64_portable.zip` 便携版。

### macOS 用户

从 [Code Go 下载页](https://shu26.cfd/download) 下载 `CodeGo_{版本号}_universal.dmg`（推荐）或 `CodeGo_{版本号}_universal.zip`。

> **注意**：`CodeGo_{版本号}_universal.app.tar.gz` 仅供应用内自动更新使用，不建议手动安装。

### Arch Linux 用户

请从 [Code Go 下载页](https://shu26.cfd/download) 下载与你架构匹配的 Linux 安装包。对 Arch 系列发行版，通常直接使用 AppImage 最省事，除非你自己维护打包脚本。

### Linux 用户

从 [Code Go 下载页](https://shu26.cfd/download) 下载最新版本的 Linux 安装包：
