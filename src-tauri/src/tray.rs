...
pub struct TrayTexts {
    ...
    pub codego_status_prefix: &'static str,
    pub codego_refresh: &'static str,
    pub codego_topup: &'static str,
    ...
    pub fn from_language(language: &str) -> Self {
        match language {
            "en" => Self {
                ...
                codego_status_prefix: "Code Go",
                codego_refresh: "Refresh Code Go balance",
                codego_topup: "Top up Code Go",
            ...
            "ja" => Self {
                ...
                codego_status_prefix: "Code Go",
                codego_refresh: "Code Go 残高を更新",
                codego_topup: "Code Go をチャージ",
            ...
            "zh-TW" => Self {
                ...
                open_website: "開啟官方網站",
                codego_status_prefix: "Code Go",
                codego_refresh: "重新整理 Code Go 餘額",
                codego_topup: "儲值 Code Go",
            ...
            _ => Self {
                ...
                open_website: "打开官方网站",
                codego_status_prefix: "Code Go",
                codego_refresh: "刷新 Code Go 余额",
                codego_topup: "充值 Code Go",
...
const CODEGO_STATUS_MENU_ID: &str = "codego_status";
const CODEGO_REFRESH_MENU_ID: &str = "codego_refresh";
const CODEGO_TOPUP_MENU_ID: &str = "codego_topup";
const CODEGO_OFFICIAL_WEBSITE: &str = "https://shu26.cfd";
...
fn format_codego_tray_status(
    tray_texts: TrayTexts,
    snapshot: &crate::commands::CodeGoTraySnapshot,
) -> String {
    if !snapshot.authenticated {
        return format!("{} · Not signed in", tray_texts.codego_status_prefix);
    }
    ...
    match snapshot.quota_usd {
        Some(balance) if snapshot.low_balance => format!(
            "{} · {} · ${balance:.2} low",
            tray_texts.codego_status_prefix, label
        ),
        Some(balance) => format!(
            "{} · {} · ${balance:.2}",
            tray_texts.codego_status_prefix, label
        ),
        None => format!("{} · {} · --", tray_texts.codego_status_prefix, label),
    }
}
...
pub fn create_tray_menu(
    ...
    if let Some(snapshot) = crate::commands::codego_tray_snapshot() {
        let status_item = MenuItem::with_id(
            app,
            CODEGO_STATUS_MENU_ID,
            format_codego_tray_status(tray_texts, &snapshot),
            false,
            None::<&str>,
        )
        .map_err(|e| AppError::Message(format!("创建 Code Go 状态菜单失败: {e}")))?;
        let refresh_item = MenuItem::with_id(
            app,
            CODEGO_REFRESH_MENU_ID,
            tray_texts.codego_refresh,
            snapshot.authenticated,
            None::<&str>,
        )
        .map_err(|e| AppError::Message(format!("创建 Code Go 刷新菜单失败: {e}")))?;
        let topup_item = MenuItem::with_id(
            app,
            CODEGO_TOPUP_MENU_ID,
            tray_texts.codego_topup,
            snapshot.authenticated,
            None::<&str>,
        )
        .map_err(|e| AppError::Message(format!("创建 Code Go 充值菜单失败: {e}")))?;
...
pub fn handle_tray_menu_event(app: &tauri::AppHandle, event_id: &str) {
    ...
    match event_id {
        ...
        "open_website" => {
            let target = crate::commands::codego_tray_snapshot()
                .map(|snapshot| snapshot.server_address)
                .unwrap_or_else(|| CODEGO_OFFICIAL_WEBSITE.to_string());
            if let Err(e) = app.opener().open_url(&target, None::<String>) {
        ...
        CODEGO_REFRESH_MENU_ID => {
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) =
                    crate::commands::refresh_codego_account_summary(Some(&app_handle), true).await
                {
                    log::warn!("刷新 Code Go 托盘余额失败: {error}");
                }
            });
        }
        CODEGO_TOPUP_MENU_ID => {
            if let Some(snapshot) = crate::commands::codego_tray_snapshot() {
                if let Err(e) = app.opener().open_url(&snapshot.topup_url, None::<String>) {
                    log::error!("打开 Code Go 充值页面失败: {e}");
                }
            }
        }
        "lightweight_mode" => {
...
mod tests {
    ...
    #[test]
    fn official_website_fallback_points_to_codego_site() {
        assert_eq!(CODEGO_OFFICIAL_WEBSITE, "https://shu26.cfd");
    }

    #[test]
    fn codego_status_formats_signed_out_state() {
        let snapshot = CodeGoTraySnapshot {
            authenticated: false,
            last_username: None,
            quota_usd: None,
            low_balance_threshold_usd: 10.0,
            low_balance: false,
            server_address: "https://shu26.cfd".to_string(),
            topup_url: "https://shu26.cfd/topup".to_string(),
        };

        let label = format_codego_tray_status(TrayTexts::from_language("en"), &snapshot);
        assert!(label.contains("Not signed in"));
    }

    #[test]
    fn codego_status_formats_low_balance() {
        let snapshot = CodeGoTraySnapshot {
            authenticated: true,
            last_username: Some("demo".to_string()),
            quota_usd: Some(3.25),
            low_balance_threshold_usd: 10.0,
            low_balance: true,
            server_address: "https://shu26.cfd".to_string(),
        ...
        let label = format_codego_tray_status(TrayTexts::from_language("en"), &snapshot);
        assert!(label.contains("Code Go"));
        assert!(label.contains("demo"));
        assert!(label.contains("$3.25"));
