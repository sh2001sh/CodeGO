...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("ccswitch://") && !url_str.starts_with("codego://") {
        return false;
    }
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                let codego_app = app_handle.clone();
                tauri::async_runtime::spawn(async move {
                    if let Err(e) =
                        crate::commands::refresh_codego_account_summary(Some(&codego_app), false)
                            .await
                    {
                        log::debug!("Code Go summary startup refresh skipped: {e}");
                    }

                    let mut interval =
                        tokio::time::interval(crate::commands::codego_summary_refresh_interval());
                    interval.tick().await;
                    loop {
                        interval.tick().await;
                        if let Err(e) =
                            crate::commands::refresh_codego_account_summary(Some(&codego_app), false)
                                .await
                        {
                            log::warn!("Code Go timed summary refresh failed: {e}");
                        }
                    }
                });
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::codego_get_auth_state,
            commands::codego_start_auth_session,
            commands::codego_poll_auth_session,
            commands::codego_logout,
            commands::codego_get_account_summary,
            commands::codego_list_authorized_devices,
            commands::codego_revoke_authorized_device,
            commands::codego_get_usage_trends,
            commands::codego_get_tokens,
            commands::codego_get_token_key,
            commands::codego_create_token,
            commands::codego_update_token,
            commands::codego_delete_token,
            commands::codego_get_usage_logs,
            commands::codego_ensure_token,
            commands::codego_get_config_template,
            commands::codego_get_config_templates,
            commands::codego_get_service_status,
            commands::codego_get_tool_config_statuses,
            commands::codego_get_tool_config_preview,
            commands::codego_apply_tool_config,
            commands::codego_apply_tool_config_from_token,
            commands::codego_restore_tool_config,
            commands::codego_test_tool_config,
            commands::codego_get_diagnostic_preview,
            commands::codego_submit_diagnostic_report,
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 codego://...）
