...
fn merge_settings_for_save(
    ...
    if incoming.codego_user_id.is_none() {
        incoming.codego_user_id = existing.codego_user_id;
    }
    if incoming.codego_server_address.is_none() {
        incoming.codego_server_address = existing.codego_server_address.clone();
    }
    if incoming.codego_last_username.is_none() {
        incoming.codego_last_username = existing.codego_last_username.clone();
    }
    incoming.codego_last_seen_quota_usd = existing.codego_last_seen_quota_usd;
...
mod tests {
    ...
    #[test]
    fn save_settings_should_preserve_codego_desktop_identity_when_payload_omits_it() {
        let existing = AppSettings {
            codego_server_address: Some("https://shu26.cfd".to_string()),
            codego_access_token: Some("token-123".to_string()),
            codego_user_id: Some(42),
            codego_last_username: Some("desk-user".to_string()),
            ..AppSettings::default()
        };
        ...
        assert_eq!(
            merged.codego_server_address.as_deref(),
            Some("https://shu26.cfd")
        );
        assert_eq!(merged.codego_access_token, None);
        assert_eq!(merged.codego_user_id, Some(42));
        assert_eq!(merged.codego_last_username.as_deref(), Some("desk-user"));
    }

    #[test]
    fn save_settings_should_preserve_codego_backend_quota_snapshot() {
        let existing = AppSettings {
            codego_last_seen_quota_usd: Some(23.5),
            ..AppSettings::default()
        };

        let incoming = AppSettings {
            codego_last_seen_quota_usd: None,
            ..AppSettings::default()
        };
        let merged = merge_settings_for_save(incoming, &existing);

        assert_eq!(merged.codego_last_seen_quota_usd, Some(23.5));
    }
