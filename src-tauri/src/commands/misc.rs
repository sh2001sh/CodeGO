...
const CODEGO_DOWNLOAD_URL: &str = "https://shu26.cfd/download";
...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(CODEGO_DOWNLOAD_URL, None::<String>)
...
mod tests {
    ...
    mod anchored_upgrade_windows {
        ...
        fn windows_no_sibling_uses_cli_update_without_package_fallback() {
            // codex 在 Windows 上刻意不走官方 self-update；缺少 sibling 包管理器时
            // 无法确认写回位置,应返回 None 让上游退回静态命令。
