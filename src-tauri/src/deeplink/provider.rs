...
fn fetch_remote_config(config_url: &str) -> Result<String, AppError> {
    validate_url(config_url, "configUrl")?;
    ...
    let response = client
        .get(config_url)
        .send()
        .map_err(|e| AppError::Message(format!("Failed to fetch remote config: {e}")))?;
