...
fn test_parse_valid_claude_deeplink() {
    ...
    assert_eq!(request.codego_action, None);
    assert_eq!(request.token_id, None);
}
...
#[test]
fn test_parse_provider_deeplink_with_codego_apply_action() {
    let url = "ccswitch://v1/import?resource=provider&app=codex&name=Code%20Go%20Codex&homepage=https%3A%2F%2Fexample.com&endpoint=https%3A%2F%2Fapi.example.com&configUrl=https%3A%2F%2Fexample.com%2Fconfig&configFormat=json&tokenId=42&codegoAction=applyToolConfig";
...
#[test]
fn test_parse_and_merge_config_remote_json() {
    ...
    let server = thread::spawn(move || {
        ...
        let body = r#"{"env":{"ANTHROPIC_AUTH_TOKEN":"sk-remote","ANTHROPIC_BASE_URL":"https://remote.example/v1","ANTHROPIC_MODEL":"claude-sonnet-4-5"}}"#;
        let response = format!(
    ...
    let request = DeepLinkImportRequest {
        ...
        config: None,
        config_format: Some("json".to_string()),
        config_url: Some(format!("http://{}/config", addr)),
        ..Default::default()
    };
    ...
    assert_eq!(merged.api_key, Some("sk-remote".to_string()));
    assert_eq!(
        merged.endpoint,
        Some("https://remote.example/v1".to_string())
    );
    assert_eq!(merged.homepage, Some("https://remote.example".to_string()));
    assert_eq!(merged.model, Some("claude-sonnet-4-5".to_string()));
}
