//! Panic Hook 模块
//!
//! 在应用崩溃时捕获 panic 信息并记录到 `<app_config_dir>/crash.log` 文件中（默认 `~/.codego/crash.log`）。
//! 便于用户和开发者诊断闪退问题。
...
/// 获取崩溃日志文件路径
pub(crate) fn get_crash_log_path() -> PathBuf {
...
mod tests {
    ...
    fn test_crash_log_path() {
        ...
        assert!(path.to_string_lossy().contains(".codego"));
    }
