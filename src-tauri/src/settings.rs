...
fn default_codego_tray_enabled() -> bool {
    true
}
...
fn default_codego_low_balance_threshold_usd() -> f64 {
    10.0
}

fn default_codego_telemetry_enabled() -> bool {
    false
}
...
/// 应用设置结构
///
/// 存储设备级别设置，保存在本地 `~/.codego/settings.json`，不随数据库同步。
/// 这确保了云同步场景下多设备可以独立运作。
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AppSettings {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub codego_server_address: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub codego_access_token: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub codego_user_id: Option<i64>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub codego_device_id: Option<i64>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    pub codego_low_balance_threshold_usd: f64,
    #[serde(default = "default_codego_telemetry_enabled")]
    pub codego_telemetry_enabled: bool,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub codego_last_seen_quota_usd: Option<f64>,
...
impl Default for AppSettings {
    fn default() -> Self {
        Self {
            ...
            language: None,
            codego_server_address: None,
            codego_access_token: None,
            codego_user_id: None,
...
impl AppSettings {
    ...
    fn normalize_paths(&mut self) {
        ...
        self.codego_server_address = self
            .codego_server_address
            .as_ref()
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.trim_end_matches('/').to_string());

        self.codego_access_token = self
            .codego_access_token
            .as_ref()
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
        ...
        self.codego_last_username = self
            .codego_last_username
            .as_ref()
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());

        if !self.codego_low_balance_threshold_usd.is_finite()
            || self.codego_low_balance_threshold_usd < 0.0
        {
            self.codego_low_balance_threshold_usd = default_codego_low_balance_threshold_usd();
        }

        self.codego_last_seen_quota_usd = self
            .codego_last_seen_quota_usd
...
pub fn update_settings(mut new_settings: AppSettings) -> Result<(), AppError> {
    ...
    new_settings.codego_access_token = None;
