...
fn cfg_path() -> PathBuf {
    ...
    PathBuf::from(home).join(".codego").join("config.json")
}
#[test]
fn load_v1_config_returns_error_and_does_not_write() {
    ...
    let bak = home.join(".codego").join("config.json.bak");
...
fn load_valid_v2_config_succeeds() {
    ...
    assert!(loaded.get_manager(&codego_lib::AppType::Claude).is_some());
