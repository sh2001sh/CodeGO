...
describe("useSettingsForm Hook", () => {
  it("should normalize settings and sync language on initialization", async () => {
    ...
    expect(settings.codegoTrayEnabled).toBe(true);
    expect(settings.codegoLowBalanceNotificationsEnabled).toBe(true);
    expect(settings.codegoLowBalanceThresholdUsd).toBe(10);
  ...
  it("should preserve explicit codego desktop settings from server data", async () => {
    useSettingsQueryMock.mockReturnValue({
      data: {
        ...
        minimizeToTrayOnClose: true,
        enableClaudePluginIntegration: false,
        codegoTrayEnabled: false,
        codegoLowBalanceNotificationsEnabled: false,
        codegoLowBalanceThresholdUsd: 4.5,
        language: "en",
      },
      isLoading: false,
    });
    ...
    expect(result.current.settings?.codegoTrayEnabled).toBe(false);
    expect(result.current.settings?.codegoLowBalanceNotificationsEnabled).toBe(
      false,
    );
    expect(result.current.settings?.codegoLowBalanceThresholdUsd).toBe(4.5);
  });
});
