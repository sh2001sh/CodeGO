...
describe("useSettings hook", () => {
  ...
  it("saves settings and flags restart when app config directory changes", async () => {
    ...
    expect(payload.codegoTrayEnabled).toBe(true);
    expect(payload.codegoLowBalanceNotificationsEnabled).toBe(true);
    expect(payload.codegoLowBalanceThresholdUsd).toBe(10);
  ...
  it("auto-saves codego desktop reminder settings", async () => {
    ...
    await act(async () => {
      await result.current.autoSaveSettings({
        ...
        codegoLowBalanceThresholdUsd: 6,
      });
    });

    const payload = mutateAsyncMock.mock.calls.at(-1)?.[0] as Settings;
    expect(payload.codegoTrayEnabled).toBe(false);
    expect(payload.codegoLowBalanceNotificationsEnabled).toBe(false);
    expect(payload.codegoLowBalanceThresholdUsd).toBe(6);
  });
