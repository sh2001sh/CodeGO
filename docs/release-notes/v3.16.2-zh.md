...
> ## 当前官方渠道（请务必阅读）
>
> codego Desktop 现在通过同一个网站账号体系、浏览器授权流程和自托管发布通道分发。下载、版本详情和问题反馈请只使用下列入口：
>
> | 类别     | 官方链接                                                                           |
> | -------- | ---------------------------------------------------------------------------------- |
> | 官网     | **[shu26.cfd](https://shu26.cfd)**                                                 |
> | 下载页   | **[shu26.cfd/download](https://shu26.cfd/download)**                               |
> | 源码     | **[github.com/sh2001sh/CodeGO](https://github.com/sh2001sh/CodeGO)**               |
> | 问题反馈 | **[github.com/sh2001sh/CodeGO/issues](https://github.com/sh2001sh/CodeGO/issues)** |
>
> 如果某个镜像站或客户端要求你在这些官方入口之外登录 codego 账号，请将其视为不可信来源，并先到网站账号页撤销对应设备再重试。

---

## 概览

codego v3.16.2 是 v3.16.1 之后的一版维护更新。在上一版集中处理 Codex 官方鉴权与本地路由接管的安全问题之后，这一版把重心放在两件事上：一是拓宽数据的可携带性和用量观测——新增 S3 兼容云同步（WebDAV 之外的第二套云备份后端）、OpenCode 会话用量同步，以及面向官方订阅的额度统计模板；二是继续打磨 Codex 通过 Chat Completions 路由第三方供应商时暴露出来的边角问题——流式截断判定、空 tools 下的 `tool_choice`、自定义工具元数据、推理 token 统计、文件 / 音频附件转换等。
...
本地代理现在会响应 Codex CLI 启动时探测的 `GET /v1/models`，返回 codego 托管的 Codex 模型目录。同时加入了过期目录守卫：解析 live 的 `config.toml`，仅当 `model_catalog_json` 仍指向 codego 持有的目录文件时才提供，避免把上一个供应商遗留的目录暴露给 Codex（[#3818](https://github.com/farion1231/cc-switch/pull/3818)）。
...
当 Chat Completions 上游在没有 `finish_reason` 或 `[DONE]` 的情况下结束流时，codego 不再把它当作正常完成：只有流真正结束才正常收尾；已产出部分内容时发出 incomplete（`max_output_tokens`）响应；完全没有产出时发出失败的 `stream_truncated` 事件。晚到的推理内容也会回填到仍在进行的流式工具调用上。
...
当 Codex 上游网关以 HTTP 413 拒绝过大的请求体时，代理现在返回专门的提示，说明这是供应商服务端的请求体大小限制（而非 codego 本地限制），并给出可操作的恢复步骤（运行 `/compact`、移除大段日志或内联图片，或请供应商调高限制），不再原样回显上游的 HTML 错误页。
...
在 Windows 上退出 codego 可能会留下一个失效的托盘图标，直到鼠标划过才消失。现在应用会在退出前显式移除托盘图标，让它随进程结束干净消失（[#3797](https://github.com/farion1231/cc-switch/pull/3797)）。

### Windows 任务栏图标

在运行时显式设置 Windows AppUserModelID，并给安装器生成的桌面和开始菜单快捷方式写入相同的 ID 和产品图标，让 codego 在任务栏上显示正确图标并正确归组（[#3457](https://github.com/farion1231/cc-switch/pull/3457)）。
...
**Codex 第三方供应商 Chat 路由**：通过 codego 本地代理把 Codex 请求转换并转发到第三方供应商时，各供应商对计费、合规与数据留存的约束不同，请在使用前阅读目标供应商的服务条款。

**Claude Desktop 第三方供应商代理切换**：通过 codego 内置代理网关把 Claude Desktop 的请求转到第三方供应商时，同样需要遵守目标供应商的计费、合规与数据留存约束。

用户启用上述功能即表示自行承担相关风险。codego 不对因使用这些功能而导致的任何账号限制、警告或服务暂停承担责任。
...
访问 [shu26.cfd/download](https://shu26.cfd/download) 获取正式下载页。它会推荐当前 Windows、macOS、Linux 构建，并在你需要更多资产时跳转到对应版本详情。
...
| 文件                               | 说明                                |
| ---------------------------------- | ----------------------------------- |
| `CodeGo_<version>_x64_en-US.msi`   | **推荐** - MSI 安装包，支持自动更新 |
| `CodeGo_<version>_arm64_en-US.msi` | Windows on ARM 安装包               |
| `CodeGo_<version>_*_portable.zip`  | 便携版，手动解压运行                |

### macOS

| 文件                                    | 说明                                          |
| --------------------------------------- | --------------------------------------------- |
| `CodeGo_<version>_universal.dmg`        | **推荐** - DMG 安装包，拖入 Applications 即可 |
| `CodeGo_<version>_universal.zip`        | 解压后拖入 Applications，通用桌面构建         |
| `CodeGo_<version>_universal.app.tar.gz` | 自动更新 / 包管理分发所用资产                 |

如果当前版本提供了 Homebrew cask，请以下载页上的链接为准，不要继续沿用旧的 `cc-switch` cask 命令。
...
| 发行版                                  | 推荐格式    | 安装方式                                                         |
| --------------------------------------- | ----------- | ---------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i CodeGo_*.deb` 或 `sudo apt install ./CodeGo_*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i CodeGo_*.rpm` 或 `sudo dnf install ./CodeGo_*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./CodeGo_*.rpm`                             |
| Arch Linux / Manjaro                    | `.AppImage` | 添加执行权限后直接运行，或直接使用对应 release 资产              |
| 其他发行版 / 不确定                     | `.AppImage` | `chmod +x CodeGo_*.AppImage && ./CodeGo_*.AppImage`              |
