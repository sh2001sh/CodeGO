...
> Codex stability patch: because some users did not want codego to change how Codex config files are written, Codex App Enhancements now has a switch and is off by default. After enabling it, you can keep using Codex mobile remote control, official plugins, and other official-app features while using third-party APIs; this release also includes a series of stability fixes.
...
> [!WARNING]
>
> ## Current Official Channels (Please Read)
>
> codego Desktop now uses the same website account, browser authorization flow, and self-hosted release channel shown below. Use these links for downloads, release details, and issue reporting:
>
> | Channel   | Official link                                                                      |
> | --------- | ---------------------------------------------------------------------------------- |
> | Website   | **[shu26.cfd](https://shu26.cfd)**                                                 |
> | Downloads | **[shu26.cfd/download](https://shu26.cfd/download)**                               |
> | Source    | **[github.com/sh2001sh/CodeGO](https://github.com/sh2001sh/CodeGO)**               |
> | Issues    | **[github.com/sh2001sh/CodeGO/issues](https://github.com/sh2001sh/CodeGO/issues)** |
>
> If a mirror or client asks you to sign in to your codego account outside these channels, treat it as untrusted and revoke the device from your website account before retrying.

---

## Overview

codego v3.16.1 is a Codex stability patch following v3.16.0. v3.16.0 promoted third-party Codex providers to first-class citizens through Chat Completions routing; this release focuses on several high-risk edges discovered in real use: official ChatGPT / Codex OAuth login state could be overwritten while switching third-party providers or during local routing takeover, the Codex model catalog could be cleared during live backfill, hot switching, takeover shutdown restore, or editing the active provider, and Codex `tool_search`, plugin / connector namespace tools, and custom tools were not fully restored back into Responses events on the Chat Completions upstream path.

This release also hardens local routing takeover ownership checks. Provider switching and takeover toggles now run serially per app. When deciding whether the live files are proxy-managed, codego no longer relies only on stale `enabled` state or whether the proxy service is currently running; it also checks backups and proxy placeholders in the live files. This prevents ordinary live writes from overwriting proxy-managed config immediately after takeover is enabled, while the proxy is temporarily stopped, or during hot switching.
...
Added an optional setting for preserving official ChatGPT / Codex OAuth login state when switching to third-party Codex providers. When enabled, codego stores third-party provider API keys in the provider-scoped `experimental_bearer_token` inside Codex `config.toml` instead of overwriting the official login cache in `auth.json`.
...
When hot switching Codex providers during local routing takeover, codego refreshes the provider id, model, and display name in the live config so the Codex client menu follows the active provider. The base URL still stays pointed at the local proxy, preventing the real upstream endpoint from leaking back into the live file.
...
When Codex is under local routing takeover, live `auth.json` / `config.toml` are temporarily rewritten by the proxy. Editing the active provider from those live files could incorrectly show proxy placeholders or official OAuth login as provider config. The editor now explicitly explains that it is showing the provider config stored in the database, not the proxy-managed live files; even if the proxy service is temporarily stopped, codego still treats the app as under takeover when the takeover state indicates so.
...
When Codex forwarding fails, codego now returns JSON errors that include provider, model, endpoint, upstream HTTP status, stable `cc_switch_*` error codes, and normalized HTTP status. This makes it much clearer which provider, endpoint, and upstream error caused the failure.
...
When local routing takeover is enabled, live `auth.json` / `config.toml` temporarily point to the codego proxy. The provider editor therefore shows the provider config saved in the database. This is expected; after takeover is disabled, codego restores live config from backups or the database source of truth.
...
**Codex third-party provider Chat routing**: when codego local proxy converts and forwards Codex requests to third-party providers, each provider may have different requirements for billing, compliance, and data retention. Read the target provider's terms before use.

**Claude Desktop third-party provider proxy switching**: when codego's built-in proxy gateway forwards Claude Desktop requests to third-party providers, you must also follow the target provider's billing, compliance, and data-retention terms.

By enabling these features, users accept the related risks. codego is not responsible for account restrictions, warnings, or service suspensions caused by using these features.
...
Visit [shu26.cfd/download](https://shu26.cfd/download) for the canonical installer page. It recommends the current Windows, macOS, and Linux builds and links to release details when you need extra assets.
...
| File                               | Description                                      |
| ---------------------------------- | ------------------------------------------------ |
| `CodeGo_<version>_x64_en-US.msi`   | **Recommended** - MSI installer with auto-update |
| `CodeGo_<version>_arm64_en-US.msi` | Windows on ARM installer                         |
| `CodeGo_<version>_*_portable.zip`  | Portable build for manual extraction             |

### macOS

| File                                    | Description                                             |
| --------------------------------------- | ------------------------------------------------------- |
| `CodeGo_<version>_universal.dmg`        | **Recommended** - DMG installer, drag to Applications   |
| `CodeGo_<version>_universal.zip`        | Unzip and drag to Applications, universal desktop build |
| `CodeGo_<version>_universal.app.tar.gz` | Auto-update / package-managed distribution artifact     |

If the current release exposes a Homebrew cask, follow the entry linked from the download page instead of relying on the old `cc-switch` cask instructions.
...
- `CodeGo_<version>_x64.AppImage` / `.deb` / `.rpm`
- `CodeGo_<version>_arm64.AppImage` / `.deb` / `.rpm`

| Distribution                            | Recommended Format | Install Command                                                     |
| --------------------------------------- | ------------------ | ------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`             | `sudo dpkg -i CodeGo_*.deb` or `sudo apt install ./CodeGo_*.deb`    |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`             | `sudo rpm -i CodeGo_*.rpm` or `sudo dnf install ./CodeGo_*.rpm`     |
| openSUSE                                | `.rpm`             | `sudo zypper install ./CodeGo_*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage`        | Make executable and run directly, or use the release asset directly |
| Other distributions / unsure            | `.AppImage`        | `chmod +x CodeGo_*.AppImage && ./CodeGo_*.AppImage`                 |
