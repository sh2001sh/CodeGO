...
> ## 現在の公式チャネル（必ずお読みください）
>
> codego Desktop は、同じ Web アカウント、ブラウザ認可フロー、自前の配布チャネルを通じて提供されています。ダウンロード、リリース詳細、問題報告は次の入口のみを利用してください：
>
> | チャネル     | 公式リンク                                                                         |
> | ------------ | ---------------------------------------------------------------------------------- |
> | 公式サイト   | **[shu26.cfd](https://shu26.cfd)**                                                 |
> | ダウンロード | **[shu26.cfd/download](https://shu26.cfd/download)**                               |
> | ソースコード | **[github.com/sh2001sh/CodeGO](https://github.com/sh2001sh/CodeGO)**               |
> | 問題報告     | **[github.com/sh2001sh/CodeGO/issues](https://github.com/sh2001sh/CodeGO/issues)** |
>
> これらの公式入口以外で codego アカウントへのサインインを求めるミラーやクライアントは信用せず、先に Web アカウント側で該当デバイスを失効させてから再試行してください。

---

## 概要

codego v3.16.2 は v3.16.1 に続くメンテナンスアップデートです。前リリースでは Codex 公式認証とローカルルーティングのテイクオーバーのセキュリティ問題に集中しましたが、本リリースは 2 点に重きを置いています。1 つ目はデータの可搬性と用量の可観測性の拡張で、S3 互換クラウド同期（WebDAV に並ぶ 2 つ目のクラウドバックアップバックエンド）、OpenCode セッション用量同期、公式サブスクリプション向けの残量統計テンプレートを追加しました。2 つ目は、Codex がサードパーティプロバイダーを Chat Completions ルーティングする際に露呈したエッジケースの継続的な改善で、ストリーム切断の判定、tools が空のときの `tool_choice`、カスタムツールのメタデータ、推論トークン統計、ファイル / 音声添付の変換などです。
...
ローカルプロキシは、Codex CLI が起動時にプローブする `GET /v1/models` に応答し、codego が管理する Codex モデルカタログを返すようになりました。あわせて古いカタログのガードを追加: live の `config.toml` を解析し、`model_catalog_json` が codego 所有のカタログファイルを指している場合のみ提供することで、前のプロバイダーが残したカタログを Codex に見せてしまうことを防ぎます（#3818）。
...
Chat Completions 上流が `finish_reason` も `[DONE]` もなくストリームを終了した場合、codego はこれを正常完了として扱わなくなりました: 本当に終了したときのみ正常に締め、部分的な出力があった場合は incomplete（`max_output_tokens`）レスポンスを、何も出力されなかった場合は失敗 `stream_truncated` イベントを発行します。遅れて届いた推論も、まだアクティブなストリーミングのツール呼び出しへバックフィルされます。
...
Codex 上流ゲートウェイが過大なリクエストボディを HTTP 413 で拒否したとき、プロキシはこれが codego のローカル制限ではなくプロバイダーのサーバー側ボディサイズ制限であることを説明する専用メッセージを返し、実行可能な回復手順（`/compact` の実行、大きなログやインライン画像の削除、プロバイダーへの上限引き上げ依頼）を提示するようになりました。上流の生の HTML エラーページをそのまま返すことはなくなりました。
...
Windows では codego を終了すると、マウスを重ねるまで無効なトレイアイコンが残ることがありました。アプリは終了前にトレイアイコンを明示的に削除するようになり、プロセス終了とともにきれいに消えます（#3797）。

### Windows タスクバーアイコン

実行時に Windows AppUserModelID を明示的に設定し、インストーラーが生成するデスクトップとスタートメニューのショートカットに同じ ID と製品アイコンを書き込みます。これにより codego がタスクバーで正しいアイコンを表示し、正しくグループ化されます（#3457）。
...
**Codex サードパーティプロバイダー Chat ルーティング**: codego ローカルプロキシで Codex リクエストを変換し、サードパーティプロバイダーへ転送する場合、課金、コンプライアンス、データ保持に関する制約はプロバイダーごとに異なります。利用前に対象プロバイダーの利用規約を確認してください。

**Claude Desktop サードパーティプロバイダープロキシ切り替え**: codego 内蔵のプロキシゲートウェイで Claude Desktop のリクエストをサードパーティプロバイダーへ転送する場合も、対象プロバイダーの課金、コンプライアンス、データ保持に関する規約に従う必要があります。

上記機能を有効化したユーザーは、関連するリスクを自ら負うものとします。codego は、これらの機能の利用によって発生したアカウント制限、警告、サービス停止について責任を負いません。
...
[shu26.cfd/download](https://shu26.cfd/download) が正規のダウンロードページです。現在の Windows、macOS、Linux 向け推奨ビルドを案内し、追加アセットが必要な場合は対応するリリース詳細へ誘導します。
...
| ファイル                           | 説明                                         |
| ---------------------------------- | -------------------------------------------- |
| `CodeGo_<version>_x64_en-US.msi`   | **推奨** - 自動更新対応の MSI インストーラー |
| `CodeGo_<version>_arm64_en-US.msi` | Windows on ARM 向けインストーラー            |
| `CodeGo_<version>_*_portable.zip`  | ポータブル版。展開してそのまま実行できます   |

### macOS

| ファイル                                | 説明                                                   |
| --------------------------------------- | ------------------------------------------------------ |
| `CodeGo_<version>_universal.dmg`        | **推奨** - DMG インストーラー。Applications へドラッグ |
| `CodeGo_<version>_universal.zip`        | 展開して Applications へドラッグするユニバーサルビルド |
| `CodeGo_<version>_universal.app.tar.gz` | 自動更新 / パッケージ管理向けの配布アセット            |

現在のリリースで Homebrew cask が提供されている場合は、旧 `cc-switch` cask コマンドではなくダウンロードページ上の案内に従ってください。
...
| --------------------------------------- | ----------- | --------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i CodeGo_*.deb` または `sudo apt install ./CodeGo_*.deb`  |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i CodeGo_*.rpm` または `sudo dnf install ./CodeGo_*.rpm`   |
| openSUSE                                | `.rpm`      | `sudo zypper install ./CodeGo_*.rpm`                                  |
| Arch Linux / Manjaro                    | `.AppImage` | 実行権限を付与して直接起動、または対応する release アセットを直接使用 |
| その他 / 不明                           | `.AppImage` | `chmod +x CodeGo_*.AppImage && ./CodeGo_*.AppImage`                   |
