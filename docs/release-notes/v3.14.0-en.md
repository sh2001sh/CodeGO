...
- **Application-Level Window Controls**: Opt-in setting to render codego's own min/max/close buttons, materially improving Linux Wayland experience
- **Hermes in Unified Skills Management**: Skill install, enable, and filter now cover Hermes
- **Hermes / OpenClaw Config Directory Override**: Point codego at a custom `~/.hermes/config.yaml` or `openclaw.json` location
...
codego now treats Hermes Agent as a first-class managed app alongside Claude / Codex / Gemini / OpenCode / OpenClaw.
...
- **User `providers:` Dict Read-Only Rendering**: User-authored providers in the YAML appear as read-only cards in codego; deep configuration delegates to the Hermes Web UI
...
- Opt-in setting to render codego's own minimize / toggle-maximize / close buttons instead of system decorations (#1119, thanks @git1677967754)
...
- New settings option to point codego at a custom `openclaw.json` location (#1518, thanks @mrFranklin)

### Hermes Config Directory Override

- New settings option to point codego at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch
...
- Deep YAML knobs are no longer duplicated in the codego form — they are delegated to the Hermes Web UI via a direct launch action
...
- Round-tripping through codego no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys
