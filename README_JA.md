...
[Code Go ダウンロードページ](https://shu26.cfd/download) から最新版の `CodeGo_{version}_x64_en-US.msi` インストーラー、または `CodeGo_{version}_x64_portable.zip` ポータブル版をダウンロードしてください。

### macOS ユーザー

[Code Go ダウンロードページ](https://shu26.cfd/download) から `CodeGo_{version}_universal.dmg`（推奨）または `CodeGo_{version}_universal.zip` をダウンロードしてください。

> **注意**: `CodeGo_{version}_universal.app.tar.gz` はアプリ内アップデーター専用であり、手動インストール用ではありません。

### Arch Linux ユーザー

[Code Go ダウンロードページ](https://shu26.cfd/download) から自分のアーキテクチャに合う Linux ビルドを取得してください。Arch 系では、自前でパッケージ管理しない限り AppImage が最も扱いやすい選択です。

### Linux ユーザー

[Code Go ダウンロードページ](https://shu26.cfd/download) から最新版の Linux ビルドをダウンロードしてください：
