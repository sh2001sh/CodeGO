...
export function DeepLinkImportDialog() {
  ...
  const isCodeGoToolApplyRequest = (
    value: DeepLinkImportRequest,
  ): value is DeepLinkImportRequest & {
    resource: "provider";
    ...
    value.codegoAction === "applyToolConfig" &&
    typeof value.tokenId === "number" &&
    Number.isFinite(value.tokenId) &&
    value.tokenId > 0 &&
    (value.app === "claude" ||
      value.app === "codex" ||
      value.app === "gemini" ||
  ...
  const handleImport = async () => {
    ...
    try {
      if (isCodeGoToolApplyRequest(request)) {
        const result = await codegoApi.applyToolConfigFromToken(
          request.tokenId,
          request.app,
        );
        await Promise.all([
  ...
  interface ParsedConfig {
    type: "claude" | "codex" | "gemini" | "opencode" | "openclaw" | "hermes";
  ...
  const parsedConfig = useMemo((): ParsedConfig | null => {
    ...
    try {
      ...
      } else if (request.app === "opencode") {
        return {
          type: "opencode",
          raw: parsed,
        };
      } else if (request.app === "openclaw") {
        return {
          type: "openclaw",
          raw: parsed,
        };
      } else if (request.app === "hermes") {
        return {
          type: "hermes",
          raw: parsed,
        };
                    ...
                    <div className="space-y-3 pt-2 border-t border-border-default">
                      ...
                      {parsedConfig && (
                        <div className="rounded-lg bg-muted/50 p-3 space-y-2">
                          ...
                          {(parsedConfig.type === "opencode" ||
                            parsedConfig.type === "openclaw" ||
                            parsedConfig.type === "hermes") && (
                            <div className="space-y-1">
                              <div className="text-xs text-muted-foreground">
                                {parsedConfig.type === "opencode"
                                  ? "OpenCode JSON:"
                              ...
                              <pre className="text-xs font-mono bg-background p-2 rounded overflow-x-auto max-h-40 whitespace-pre-wrap">
                                {JSON.stringify(parsedConfig.raw, null, 2)}
                              </pre>
                            </div>
                          )}
                        </div>
