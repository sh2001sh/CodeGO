...
async function createFixture() {
  ...
  await Promise.all([
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_x64_en-US.msi'), 'windows-x64', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_x64_en-US.msi.sig'), 'sig-win-x64\n', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_arm64_en-US.msi'), 'windows-arm64', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_arm64_en-US.msi.sig'), 'sig-win-arm64\n', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_x64_portable.zip'), 'portable', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_universal.dmg'), 'mac-dmg', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_universal.zip'), 'mac-zip', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_universal.app.tar.gz'), 'mac-updater', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_universal.app.tar.gz.sig'), 'sig-mac\n', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_x64.AppImage'), 'linux-appimage', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_x64.AppImage.sig'), 'sig-linux\n', 'utf8'),
    writeFile(path.join(assetsDir, 'CodeGo_3.16.4_x64.deb'), 'linux-deb', 'utf8'),
  ]);
  return root;
}
...
describe('build-codego-release-manifest', () => {
  test('generates release manifest and updater manifest from release assets', async () => {
    ...
    assert.equal(
      manifest.assets.find((asset) => asset.name === 'CodeGo_3.16.4_x64_en-US.msi')?.digest,
      sha256('windows-x64')
    );
    assert.equal(
      manifest.assets.find((asset) => asset.name === 'CodeGo_3.16.4_universal.app.tar.gz')?.tauri_target,
      'darwin-universal'
    );
    assert.equal(
