...
const PRODUCT_NAME = 'Code Go Desktop';
...
export async function buildReleaseManifest({
  ...
  const assets = await Promise.all(
    fileEntries.map((entry) => buildAssetRecord(assetsDir, entry, downloadBaseURL))
  );
  ...
  const resolvedPublishedAt = String(publishedAt || '').trim() || new Date().toISOString();
  const resolvedNotes =
  ...
  const manifest = {
    ...
    html_url:
      String(releasePageURL || '').trim() || `/download?version=${normalizedTag}`,
...
export async function runCLI(argv = process.argv.slice(2)) {
  const { values } = parseArgs({
    ...
    options: {
      ...
      'manifest-out': { type: 'string', default: 'release-assets/codego-desktop-release-manifest.json' },
